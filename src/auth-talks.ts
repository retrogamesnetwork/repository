...
export async function requestLogin() {
    try {
        ...
        const response = await getObject(ssoUrl + "?url=" + backUrl);
...
export async function requestLogout(user: User | null): Promise<void> {
    if (user !== null && user.namespace === "doszone") {
        await getObject(ssoLogout + "?sso=" + user.sso + "&sig=" + user.sig).catch(console.error);
    }
}
...
async function validateUser(user: User): Promise<User | null> {
    try {
        return (await getObject(ssoLogin + "?sso=" + user.sso +
